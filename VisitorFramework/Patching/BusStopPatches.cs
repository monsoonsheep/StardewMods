...
internal class BusStopPatches : PatchCollection
{
    ...
    internal BusStopPatches()
    {
        Patches = new List<Patch>
        {
            new(
                ...
                "doorOpenAfterReturn",
                null,
                postfix: nameof(BusStopDoorOpenAfterReturn)),
            new(
                typeof(BusStop),
    ...
    /// </summary>
    private static void BusStopDoorOpenAfterReturn(BusStop __instance)
    {
        if (Bm.BusGone)
        {
            Bm.SetBusOutOfFrame();
    ...
    private static bool BusStopAnswerDialoguePrefix(BusStop __instance, Response answer, ref bool __result)
    {
        ...
        if (Bm.BusLeaving || Bm.BusReturning || (Bm.BusGone && (Bm.TimeUntilNextArrival <= 20 || Bm.TimeSinceLastArrival <= 20)))
        {
    ...
    private static void BusStopAnswerDialoguePostfix(BusStop __instance, Response answer, ref bool __result)
    {
        if (__result == true && Game1.player.controller != null)
        {
            ...
            {
                if (Bm.TimeUntilNextArrival > 20 && Bm.TimeSinceLastArrival > 20)
                {
