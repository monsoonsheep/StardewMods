...
namespace VisitorFramework.Framework.Visitors
{
    public static class NpcExtensions
    {
            ...
            PathFindController.endBehavior endBehavior = null)
        {
            ...
            {
                Log.Debug("Invalid location in pathfinding..", LogLevel.Error);
                return;
            }
            me.controller = null;
            me.isCharging = false;
            ...
            {
                Log.Debug("Character couldn't find path.", LogLevel.Warn);
                return;
            }
            ...
            {
                Log.Debug("Can't construct controller.", LogLevel.Error);
                //GoHome();
            }
        ...
        public static Stack<Point> PathTo(this NPC me, GameLocation startingLocation, Point startTile, GameLocation targetLocation,
            Point targetTile)
        {
            ...
            {
                if (!targetLocation.isTilePassable(new Location(targetTile.X, targetTile.Y), Game1.viewport)
            ...
            string[] locationsRoute = VisitorManager.GetLocationRoute(startingLocation, targetLocation);
        ...
        public static Stack<Point> FindPathInLocation(this NPC me, Point startTile, Point targetTile, GameLocation location, bool pathingToObject = false, int iterations = 30000)
        {
            if (pathingToObject)
            {
            ...
            if (location is Farm || (location.GetContainingBuilding() is { } containing && containing.GetParentLocation() is Farm))
            {
            ...
            if (path == null)
            {
                Log.Debug($"Couldn't find path in {location.Name}");
            }
            return path;
        }

        public static Stack<Point> PathToObject(this NPC me, GameLocation location, Point startTile, Point targetTile)
        {
