...
internal sealed class TableManager
{
    ...
    internal readonly List<Table> CurrentTables = new();
    internal readonly Dictionary<Rectangle, List<Vector2>> MapTablesInCafeLocation = new();

    internal TableManager() => Instance = this;
    ...
    internal Seat GetSeatAt(GameLocation location, Point position)
    {
        ...
        .FirstOrDefault(seat => seat.Position.X == position.X && seat.Position.Y == position.Y);
    }
